...
test.describe("Volunteer Filtering Logic", () => {
  ...
  test("should combine location and availability filters", async ({
    request,
  }) => {
    const response = await request.get("/api/admin/volunteers", {
      headers: {
  ...
  test("should respect notification opt-out regardless of other filters", async ({
    request,
  }) => {
  ...
  test("should handle null/empty availability gracefully", async ({
    request,
  }) => {
  ...
  test("should return consistent results for repeated queries", async ({
    ...
    const volunteerCounts = datasets.map(
      (d) =>
        d.volunteers.filter((v: any) => volunteerEmails.includes(v.email))
          .length
    );

    expect(
      volunteerCounts.every((count) => count === volunteerCounts[0])
    ).toBeTruthy();
  ...
  test("should handle malformed JSON in database fields", async ({
    request,
  }) => {
    ...
    const corruptVolunteer = data.volunteers.find(
      (v: any) => v.email === corruptVolunteerEmail
    );
