...
test.describe('Admin Shift Shortage Notifications', () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    // Volunteer 2: Glenn Innes, opted in
  ...
  test('should load notifications page and show basic filters', async ({ page }) => {
  ...
  test('should show volunteer count when filters are applied', async ({ page }) => {
    await page.goto('/admin/notifications');
    ...
    await expect(page.getByTestId('volunteer-count')).toContainText('volunteers match filters');
  });
  ...
  test('should show availability filter for selected shift', async ({ page }) => {
    await page.goto('/admin/notifications');
