...
test.describe('Admin Navigation', () => {
  ...
  test.describe('Admin Sidebar Navigation', () => {
    test('admin sees all navigation links in sidebar', async ({ page }) => {
      ...
      await expect(page.getByTestId('sidebar-dashboard')).toContainText('Dashboard');
      
      const sidebar = page.getByTestId('admin-sidebar');
      await expect(sidebar.getByTestId('manage-users-button')).toBeVisible();
      await expect(sidebar.getByTestId('manage-users-button')).toContainText('All Users');
      
      await expect(page.getByTestId('sidebar-regulars')).toBeVisible();
      await expect(page.getByTestId('sidebar-regulars')).toContainText('Regular Volunteers');
      
      await expect(page.getByTestId('sidebar-restaurant-managers')).toBeVisible();
      await expect(page.getByTestId('sidebar-restaurant-managers')).toContainText('Restaurant Managers');
      
      await expect(page.getByTestId('sidebar-shifts/new')).toBeVisible();
      await expect(page.getByTestId('sidebar-shifts/new')).toContainText('Create Shift');
      
      await expect(sidebar.getByTestId('manage-shifts-button')).toBeVisible();
      await expect(sidebar.getByTestId('manage-shifts-button')).toContainText('Manage Shifts');
      
      // Migration tab links
      await expect(page.getByTestId('sidebar-migration#upload')).toBeVisible();
      await expect(page.getByTestId('sidebar-migration#upload')).toContainText('Upload CSV');
      
      await expect(page.getByTestId('sidebar-migration#status')).toBeVisible();
      await expect(page.getByTestId('sidebar-migration#status')).toContainText('Migration Status');
      
      await expect(page.getByTestId('sidebar-migration#invitations')).toBeVisible();
      await expect(page.getByTestId('sidebar-migration#invitations')).toContainText('User Invitations');
      
      await expect(page.getByTestId('sidebar-migration#users')).toBeVisible();
      await expect(page.getByTestId('sidebar-migration#users')).toContainText('Migrated Users');

      // Public shifts link that opens in new tab
      await expect(sidebar.getByTestId('view-public-shifts-button')).toBeVisible();
    ...
    test('migration upload link navigation works correctly', async ({ page }) => {
      // Login as admin
      await loginAsAdmin(page);
      ...
      const migrationUploadLink = page.getByTestId('sidebar-migration#upload');
    ...
    test('sidebar navigation works from different admin pages', async ({ page }) => {
      // Login as admin
      await loginAsAdmin(page);
