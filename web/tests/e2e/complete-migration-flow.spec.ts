...
test.describe("Complete Migration Flow", () => {
  ...
  test.describe("Admin Migration Management", () => {
    test("admin can access migration page via header link", async ({
      page,
    }) => {
      // Login as admin
    ...
    test("admin can view migrated users and generate invitation", async ({
      page,
    }) => {
      ...
      const sendInviteButton = userRow.locator(
      ...
      await expect(
        page.locator('.toast, [data-testid="success-message"]')
      ).toContainText(/invitation sent|success/i);
    });
  });

  test.describe("Migration Registration Flow", () => {
    test("volunteer can complete full 6-step migration registration", async ({
      page,
    }) => {
      ...
      // Verify success
      await expect(
        page.locator("text=Registration completed successfully")
      ).toBeVisible();
    ...
    test("migration registration requires profile photo", async ({ page }) => {
      ...
      await expect(
        page.locator(
          "text=Profile image is required for migration registration"
        )
      ).toBeVisible();

      // Verify we can't proceed
      await expect(
        page.locator('h2:has-text("Availability")')
      ).not.toBeVisible();
    });
    ...
    test("migration handles expired tokens", async ({ page }) => {
      ...
      await expect(
        page.locator("text=Migration link has expired")
      ).toBeVisible();
      await expect(
        page.locator("text=Please contact an administrator")
      ).toBeVisible();
    });
  ...
  test.describe("Migration Status Updates", () => {
    test("admin can see updated migration stats after completion", async ({
      page,
    }) => {
      ...
      const completedCount = page.locator(
        '[data-testid="completed-registrations-count"]'
      );
