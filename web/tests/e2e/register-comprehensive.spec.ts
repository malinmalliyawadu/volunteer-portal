...
async function fillPersonalInfoStep(
  ...
  if (pronouns !== "none") {
    ...
    if (await pronounsSelect.isVisible()) {
      ...
      await page
        .getByRole("option", { name: new RegExp(pronouns, "i") })
        .click();
    }
  }
}
...
async function fillMedicalInfoStep(
  ...
  if (medicalConditions) {
    await page
      .getByRole("textbox", { name: /medical conditions/i })
      .fill(medicalConditions);
  }

  if (willingToProvideReference) {
    await page
      .getByRole("checkbox", { name: /willing to provide reference/i })
      .check();
  }
  ...
  if (howDidYouHearAboutUs !== "not_specified") {
    ...
    if (await hearAboutSelect.isVisible()) {
      ...
      await page
        .getByRole("option", {
          name: new RegExp(howDidYouHearAboutUs.replace("_", " "), "i"),
        })
        .click();
    }
  }
...
async function fillAvailabilityStep(
  ...
  for (const location of locations) {
    const locationCheckbox = page.getByTestId(
      `available-location-${location.toLowerCase()}`
    );
...
async function fillCommunicationStep(
  ...
  if (notificationPreference !== "EMAIL") {
    const notificationSelect = page.getByTestId(
      "notification-preference-select"
    );
    if (await notificationSelect.isVisible()) {
      await notificationSelect.click();
      await page
        .getByRole("option", { name: new RegExp(notificationPreference, "i") })
        .click();
    }
  }
  ...
  if (acceptVolunteerAgreement) {
    const volunteerAgreementCheckbox = page.getByRole("checkbox", {
      name: /volunteer agreement/i,
    });
  ...
  if (acceptHealthSafetyPolicy) {
    const healthSafetyCheckbox = page.getByRole("checkbox", {
...
test.describe("Registration Page - Comprehensive Tests", () => {
  ...
  test.describe("Page Layout and Structure", () => {
    ...
    test("should update progress as user advances through steps", async ({
      ...
      await expect(page.getByTestId("current-step-title")).toContainText(
  ...
  test.describe("OAuth Provider Integration", () => {
    ...
    test("should handle OAuth provider clicks without errors", async ({
      page,
    }) => {
      const oauthSection = page.getByTestId("oauth-providers");
  ...
  test.describe("Step 1 - Account Creation", () => {
    test("should display all account creation form fields", async ({
      page,
    }) => {
      // Welcome message
  ...
  test.describe("Step 2 - Personal Information", () => {
    ...
    test("should display personal information form fields", async ({
      page,
    }) => {
      // Name fields
  ...
  test.describe("Step 3 - Emergency Contact", () => {
    ...
    test("should allow skipping emergency contact (optional fields)", async ({
      page,
    }) => {
  ...
  test.describe("Step 4 - Medical Information & Background", () => {
    ...
    test("should display medical information form fields", async ({ page }) => {
      ...
      const medicalField = page.getByRole("textbox", {
        name: /medical conditions/i,
      });
      await expect(medicalField).toBeVisible();

      // Reference checkbox
      const referenceCheckbox = page.getByRole("checkbox", {
        name: /willing to provide reference/i,
      });
    ...
    test("should handle 'how did you hear about us' selection", async ({
      page,
    }) => {
      const hearAboutSelect = page.getByTestId("how-did-you-hear-select");
      await hearAboutSelect.click();
  ...
  test.describe("Step 5 - Availability", () => {
    ...
    test("should display availability form fields", async ({ page }) => {
      ...
      const locationsLabel = page.getByText(
        /locations where you can volunteer/i
      );
      ...
      for (const location of locations) {
        const locationCheckbox = page.getByTestId(
          `available-location-${location.toLowerCase().replace(" ", "_")}`
        );
    ...
    test("should allow selecting available locations", async ({ page }) => {
      ...
      const wellingtonCheckbox = page.getByTestId(
        "available-location-wellington"
      );
  ...
  test.describe("Step 6 - Final Steps / Agreements", () => {
    ...
    test("should display communication preferences and agreements", async ({
      page,
    }) => {
      ...
      const volunteerAgreementLink = page.getByRole("button", {
        name: /volunteer agreement/i,
      });
      await expect(volunteerAgreementLink).toBeVisible();

      const healthSafetyLink = page.getByRole("button", {
        name: /health and safety/i,
      });
      await expect(healthSafetyLink).toBeVisible();
    });

    test("should require accepting agreements before submission", async ({
      page,
    }) => {
    ...
    test("should show Create Account button on final step", async ({
      page,
    }) => {
      const submitButton = page.getByTestId("next-submit-button");
      await expect(submitButton).toContainText(/create account/i);
    ...
    test("should handle notification preference selection", async ({
      page,
    }) => {
      const notificationSelect = page.getByTestId(
        "notification-preference-select"
  ...
  test.describe("Navigation and Data Persistence", () => {
    ...
    test("should preserve form data when navigating back and forth", async ({
      ...
      ).toHaveValue(testFirstName);
      await expect(
        page.getByRole("textbox", { name: /last name/i })
      ).toHaveValue(testLastName);
    });
  ...
  test.describe("Complete Registration Flow", () => {
    ...
    test("should validate comprehensive form data before submission", async ({
      page,
    }) => {
  ...
  test.describe("Link Navigation", () => {
    ...
    test("should open policy dialogs when clicking agreement links", async ({
      page,
    }) => {
      await navigateToStep(page, 6);
      ...
      const healthSafetyDialog = page.getByRole("dialog", {
        name: /health and safety/i,
      });
